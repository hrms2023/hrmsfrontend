...
import { AdminPanelSidebarDetails } from "../../app/utils";
import { ChevronRightIcon } from "@heroicons/react/24/solid";
import { useRouter } from 'next/navigation';


const AdminPanelSidebar = ({ isMenuOpen }) => {
            ...
            <div className={`flex h-screen ${isMenuOpen ? 'overflow-hidden' : ''}`}>
                ...
                >
                    <div className="flex items-center justify-between p-4 flex-col">
                        <span className="text-black text-[12px] font-semibold "></span>
                    </div>
                    <div className={`p-4 ${isMenuOpen ? 'block' : 'hidden'}`}>
                        <ul className="space-y-2 text-gray-900">
                            {AdminPanelSidebarDetails.map((list, id) => (
                                <div
                                    key={id}
                                ...
                                >
                                    <div className='flex flex-row justify-between'>
                                        ...
                                        >
                                            {list.icons === "" ? null : <img src={list.icons} className='h-4 w-4' alt='img' />}
                                        ...
                                        <ul  className="pl-6 m-3 ">
                                            {list.items.map((item, index) => (
                                                <li onClick={(()=> navigateToPage(item.route))} key={index} className="hover:text-gray-700 text-[12px] font-medium font-roboto">
                                                    <a href="#">{item.name}</a>
                                                    <div className="border-b mt-1.5 mb-2 p-1 border-gray-400 w-full"></div>
                                                </li>
                                            ))}
                                        </ul>
                                    )}
                        ...
                        <div className='flex flex-col items-center text-center justify-center mt-10'>
                            <div className='flex flex-row'>
                               <p className='text-[12px]'>©</p>
                                ...
                                <p className='text-[12px] font-semibold ml-1'>ArtKraft</p>
                            </div>
                            <div className='flex flex-row'>
